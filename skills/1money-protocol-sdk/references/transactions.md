...
Batch Payment is the SDK's canonical native-v2 bulk-transfer flow. There is no
`legacyV1.batchPayment` or legacy Batch Payment builder exposed by this SDK,
even if a node retains a deprecated v1 route. Use the full estimate → prepare →
sign → authorize → submit → read lifecycle:
...
// This is an ordinary promise-wrapper request, not a v2 submission. The
// point-in-time quote is unsigned, non-binding, and never becomes a fee cap.
...
The estimator posts exactly this unsigned body to
...
The canonical write is `POST /v2/transactions/batch_payment`; it receives only
the authorized request generated by this builder. Never hand-write the wire
body or add the obsolete `max_fee` field. The actual sender debit is the sum of
operation amounts plus the actual, decimal-string receipt `fee_used`; the quote
is not signed and need not equal that final fee.
...
the option to sign and send `{ type: '', format: '', data: '' }`; the empty
strings are still a signed and transmitted value. `operations_hash` is optional
and the helper never inserts it automatically, but if you supply it, prepare
...
and wire body. A present `operations_hash` must be a `0x…` 32-byte hash and a
present `batch_id` a string. `batch_id` is correlation metadata only; it has no
uniqueness, deduplication, idempotency, or replay guarantee. Use the fresh
transaction nonce for transaction-level replay protection.

The SDK validates static syntax, nonzero recipients and amounts, and U256
total overflow locally. Batch enablement, operation-count limit, encoded-size
limit, and fee asset are governance-controlled by the node and remain
server-authoritative. A failed operation atomically rolls back every recipient
credit, sender debit, and operator-fee movement.

When you read the receipt, treat `success_info.receiver` as a zero-address
sentinel for Batch Payment, not a recipient. The actual ordered recipients and
